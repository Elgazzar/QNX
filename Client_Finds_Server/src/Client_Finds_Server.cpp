...
void *server_thread(void *arg) {
  ...
  cout << "[Server] Registering global name: \"" << ATTACH_POINT << "\""
       << endl;
  ...
  while (true) {
    ...
    // name_attach channels also receive system pulses (like when clients
    // connect/disconnect)
    if (rcvid == 0) {
      struct _pulse *pulse = (struct _pulse *)&msg;
      if (pulse->code == CKSUM_PULSE_CODE) {
        cout << "\n[Server] Received a CKSUM_PULSE_CODE! Value: "
             << pulse->value.sival_int << endl;
      } else if (pulse->code == _PULSE_CODE_DISCONNECT) {
        cout << "[Server] Received a disconnect pulse from the OS. Cleaning up "
                "connection."
             << endl;
        ConnectDetach(pulse->scoid); // VERY IMPORTANT: Free up server resources
                                     // when client detaches
      } else {
        cout << "[Server] Received a system pulse. Code: " << (int)pulse->code
             << endl;
      }
    } else {
...
void *client_thread(void *arg) {
  ...
  cout << "\n[Client] Looking for server named: \"" << ATTACH_POINT << "\"..."
       << endl;
  ...
  cout << "[Client] Connected successfully! I got a standard Connection ID: "
       << coid << endl;

  // --- NEW: Send a pulse ---
  cout << "\n[Client] Sending a fast, non-blocking pulse..." << endl;
  MsgSendPulse(coid, 10, CKSUM_PULSE_CODE, 777);
  sleep(1); // Give server a moment to print the pulse before we send the next
            // message
